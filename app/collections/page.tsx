...
export default function CollectionsPage() {
  const [selected, setSelected] = useState<Record<string, string>>({ category: "All", price: "All", color: "All", fabric: "All", occasion: "All" });
  const [sort, setSort] = useState("Newest");
  const searchParams = useSearchParams();
  const categoryFromQuery = searchParams.get("category") || "All";
  const occasionFromQuery = searchParams.get("occasion") || "All";
        ...
        <div className="flex flex-col justify-between gap-5 border-b hairline pb-8 md:flex-row md:items-end">
          ...
          <label className="flex items-center gap-3 border hairline px-4 py-3">
            ...
            <select value={sort} onChange={event => setSort(event.target.value)} className="bg-transparent outline-none">
              {["Newest", "Popular", "Price Low-High", "Price High-Low"].map(option => <option key={option}>{option}</option>)}
            </select>
          </label>
        </div>
        <div className="mt-8 grid gap-8 lg:grid-cols-[280px_1fr]">
          <aside className="glass h-fit p-5">
            <h2 className="flex items-center gap-2 font-display text-xs uppercase tracking-[.24em] text-gold"><SlidersHorizontal size={16} /> Filters</h2>
